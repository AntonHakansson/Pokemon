...
const favoritesIcon = {index: 2, div: <FontIcon className="material-icons">favorite</FontIcon>};
...
class PokedexViewer extends React.Component {
  ...
  fetchPokemons(){
    ...
    Pokedex.getPokemonsList().then(ress => {
      let pokemons = ress.results.slice(0, 10);
      ...
      if (pokemon){
        return(
          <GridTile key={pokemon.name}
                     title={pokemon.name}
                      subtitle={<span>{pokemon.base_experience} CP<br/>{pokemon.types.map(type => {return type.type.name + " ";})}</span>}
            ...
            <Dialog
              ...
              onRequestClose={() => {pokemon.dialogOpen = false; this.forceUpdate();}} >
                <span>Base CP: {pokemon.base_experience}</span>
                {dialogDivider}
